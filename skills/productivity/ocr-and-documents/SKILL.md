...
For DOCX: use `python-docx` if available. If lxml import fails in sandbox environments, fall back to zipfile+xml.etree (zero dependencies, always works):
...
- For Word docs: try `python-docx` first; if lxml fails in sandbox, use zipfile fallback:
  ```python
